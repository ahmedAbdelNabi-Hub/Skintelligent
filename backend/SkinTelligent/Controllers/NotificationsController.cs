...
namespace SkinTelligent.Api.Controllers
{
    ...
    public class NotificationsController : BaseController
    {
        ...
        public async Task<ActionResult<BaseApiResponse>> MarkAllNotificationsAsRead()
        {
            var userId = GetAuthenticatedUserId();
            if (string.IsNullOrEmpty(userId))
                return Unauthorized(new BaseApiResponse(401, "Unauthorized."));

            var spec = new NotificationSpecification(userId, onlyUnread: true);
            var unreadNotifications = await _unitOfWork.Repository<Notification>().GetAllWithSpecAsync(spec,true);

            if (unreadNotifications == null || unreadNotifications.Count == 0)
                return Ok(new BaseApiResponse(200, "No unread notifications."));
            ...
            if (!success)
                return StatusCode(500, new BaseApiResponse(500, "Failed to mark all notifications as read."));

            return Ok(new BaseApiResponse(200, "All notifications marked as read."));
        }
