...
namespace SkinTelligent.Api.Controllers
{
        ...
        public async Task<IActionResult> GetDoctorPatients([FromQuery] PaginationSpecParams paginationParams)
    {
        ...
        public async Task<ActionResult<PaginationDTO<ClinicDTO>>> GetDoctorClinics(int? id, [FromQuery] PaginationSpecParams paginationParams)
        {
            ...
            {
                if (id == null)
                    return BadRequest("Doctor ID is required for patients.");
                finalDoctorId = id.Value;
            }
            else
            {
            ...
            var spec = new ClinicDoctorSpecifications(finalDoctorId.Value, paginationParams);
            var doctorClinics = await _unitOfWork.Repository<ClinicDoctor>().GetAllWithSpecAsync(spec);

            if (doctorClinics == null || !doctorClinics.Any())
                return NotFound("Doctor not found or no clinics associated with the doctor.");

            var mappedClinics = _mapper.Map<IReadOnlyList<ClinicDTO>>(doctorClinics.Select(dc => dc.Clinic));

            var countSpec = new ClinicDoctorSpecifications(finalDoctorId.Value, paginationParams);
            var totalCount = await _unitOfWork.Repository<ClinicDoctor>().CountWithSpec(countSpec);

            return Ok(new PaginationDTO<ClinicDTO>
            {
                data = mappedClinics,
                PageIndex = paginationParams.PageIndex,
                PageSize = paginationParams.PageSize,
